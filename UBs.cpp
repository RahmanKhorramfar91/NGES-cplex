...
#pragma region Electricity Network Costs
	...
	exp_Elec = ex_est + ex_decom + ex_fix + ex_var + ex_fuel + ex_emis + ex_shedd + ex_trans + ex_elec_str;
...
#pragma region NG network related costs
	...
	exp_NG = exp_NG_import_cost + exp_invG + exp_pipe + exp_curt + exp_FixVar;
...
#pragma endregion
	IloNumVar Elec_total(env, 0, IloInfinity, ILOFLOAT);
	IloNumVar NG_total(env, 0, IloInfinity, ILOFLOAT);
	Model.add(Elec_total == exp_Elec);
	Model.add(NG_total == exp_NG);
	exp0 = exp_Elec + exp_NG;
...
#pragma region print Electricity Network variables
		...
		fid << "\t Electricity Network Obj Value:" << cplex.getValue(Elec_total) << endl;
...
#pragma region print NG network variables
		...
		fid2 << "\t NG Network Obj Value:" << cplex.getValue(NG_total) << endl;
...
#pragma region Electricity Network Costs
	...
	for (int n = 0; n < nEnode; n++)
	{
		...
		for (int t = 0; t < Te.size(); t++)
		{
			...
			{
				// var cost
				ex_var += time_weight[t] * Plants[i].var_cost * Ps[n][t][i];

				// fuel price to be updated later (dollar per thousand cubic feet=MMBTu)
				if (Plants[i].type == "ng" || Plants[i].type == "CT" || Plants[i].type == "CC" || Plants[i].type == "CC-CCS")
				{
					ex_fuel += time_weight[t] * NG_price * Plants[i].heat_rate * Ps[n][t][i];
				}
				else if (Plants[i].type == "dfo")
				{
					ex_fuel += time_weight[t] * dfo_pric * Plants[i].heat_rate * Ps[n][t][i];
				}
				else if (Plants[i].type == "coal")
				{
					ex_fuel += time_weight[t] * coal_price * Plants[i].heat_rate * Ps[n][t][i];
				}

				// emission cost (to be added later)
				ex_emis += time_weight[t] * Plants[i].emis_cost * Plants[i].emis_rate * Ps[n][t][i];
			}
...
#pragma region Electricity Network Constraints
	...
	for (int br = 0; br < Branches.size(); br++)
	{
		...
		for (int t = 0; t < Te.size(); t++)
		{
			if (Branches[br].is_exist == 1)
			{
				Model.add(flowE[br][t] - Branches[br].suscep * (theta[tb][t] - theta[fb][t]) <= 3);
				Model.add(3 <= flowE[br][t] - Branches[br].suscep * (theta[tb][t] - theta[fb][t]));
			}
			else
			{
				Model.add(flowE[br][t] - Branches[br].suscep * (theta[tb][t] - theta[fb][t]) <= 1e-2 + Branches[br].suscep * 24 * (1 - Ze[br]));
			...
			for (int n : Gnodes[k].adjE)
			{
				...
				for (int t = tau * 24; t < (tau + 1) * 24; t++)
				{
					for (int i = 0; i < nPlt; i++)
					{
						if (Plants[i].type == "ng" || Plants[i].type == "CT" || Plants[i].type == "CC" || Plants[i].type == "CC-CCS")
						{
							exp2 += Plants[i].heat_rate * Ps[n][t][i];
						}
					}
				}
				Model.add(flowGE[k][n][tau] - exp2 <= 1e-2);
