...
{2,"solar"},{3,"wind"},{4,"wind_offshore"},{5,"hydro"},{6,"coal"},{7,"nuclear"} };
	...
	vector<branch> Branches = branch::read_branch_data(nEnode, "b2b_br_per_node.txt", "b2b_br.txt", "b2b_br_dist.txt",
		"b2b_br_is_existing.txt", "b2b_br_maxFlow.txt", "b2b_br_Suscept.txt");

	int nPlt = sym2pltType.size();
...
#pragma region temp parameters to be revised later
...
#pragma region Electricity Network Constraints
	...
	for (int n = 0; n < nEnode; n++)
	{
		...
		for (int i = 0; i < nPlt; i++)
		{
			// wind_offshore plants are not allowed
...
#pragma region Solve the model
	...
	float CORP_obj = cplex.getObjValue();
	...
	std::cout << "\t Obj Value:" << CORP_obj << endl;
...
#pragma region print Electricity Network variables
	...
	fid << "\t RMP Obj Value:" << CORP_obj << endl;
